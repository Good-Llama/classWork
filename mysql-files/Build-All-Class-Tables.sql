...
-- ALTER will change the structure of a table without dropping or recreating
-- You can pretty much ALTER anything, add cols, remove cols, add/remove primary key or foreign key

ALTER TABLE User AUTO_INCREMENT = 10293;  -- Set starting uid value instead of start at 1 and going up by 1, we can change it

insert into User
  (name, phone, email)  -- uid col is not included since it is automatically generated
...
create table Customer (  -- create a new table Customer
    ...
    uid   int NOT NULL UNIQUE,             -- Associated User id - must match a row in the User table
    address varchar(256),
...
create table Product (
    ...
    name    varchar(256) NOT NULL,
    brand   varchar(256),
    price   float default 0.00, -- default tells DMMS to provide a value if not given when row added
    color   varchar(256),
    ratings float
    );
 show tables;
...
-- Customer is a dependent of User (established by FOREIGN KEY clause on the create table Customer)
--      the uid col in Customer table must match an existing uid in the User table
--
-- The uid in the User table is automatically generated
--      we don't know what it is for a specific user
--      but the DBMS know the uid for each user
--      so we will ask the DBMS for the uid in User for a specific user with a SELECT on the INSERT
--          when we add an associated customer to the Customer table
-- instead of a values() clause to specify the values for the columns, we use a SELECT
-- SELECT - retrieve values from a table 

insert into Customer
  (uid, address)
-- retrieve the uid, 'use this literal' from the User table where the name is Frank
  select uid, '1 Java Ln, Phoenix AZ 85339' from User where name ='Frank'
;

-- LIKE does a wildcard search of table:
--
-- % is the wildcard character and means 0 to any number of characters
--
-- starts-with: 'word%'
-- ends-with: '%word'
-- contains: '%word%'
